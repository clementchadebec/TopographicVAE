...
def create_model(n_caps, cap_dim, mu_init, n_transforms, group_kernel, n_off_diag):
    ...
    z_encoder = Gaussian_Encoder(Encoder_ColorMNIST(s_dim=s_dim, n_cin=3, n_hw=28),
                                 loc=0.0, scale=1.0)

    u_encoder = Gaussian_Encoder(Encoder_ColorMNIST(s_dim=s_dim, n_cin=3, n_hw=28),                                
                                 loc=0.0, scale=1.0)

    decoder = Gaussian_Decoder(Decoder_ColorMNIST(s_dim=s_dim, n_cout=3, n_hw=28))
...
def main(args):
    config = {
        ...
        'mu_init': 30.0,
        'n_off_diag': 1,
        'group_kernel': (12, 12, 1),
        'n_is_samples': 100,
        "prob_missing_data": args.prob_missing_data,
        "prob_missing_pixels": args.prob_missing_pixels
        }

    name = f'TVAE_COLORMNIST_L=1/2_K=3_pix_{config["prob_missing_pixels"]}_data_{config["prob_missing_data"]}'
    ...
    train_data = torch.load(os.path.join('/gpfswork/rech/wlr/uhw48em/rvae/data/colormnist/train_12_long_color_mnist.pt'), map_location="cpu")#[:-1000]
    eval_data = torch.load(os.path.join('/gpfswork/rech/wlr/uhw48em/rvae/data/colormnist/val_12_long_color_mnist.pt'), map_location="cpu")#[-1000:]
    ...
    masks = np.load(f"/gpfswork/rech/wlr/uhw48em/rvae/data/colormnist/masks/mask_miss_data_{args.prob_missing_data}_miss_pixels_{args.prob_missing_pixels}.npz")
    ...
    print("------------------------------------------------------------")
    print("Dataset \t \t Shape \t \t \t Range")
    print(
        f" train data: \t {train_data.shape} \t [{train_data.min()}-{train_data.max()}] "
    )
    print(
        f" eval data: \t {eval_data.shape} \t [{eval_data.min()}-{eval_data.max()}]"
    )
    print(
        f" test data: \t {test_data.shape} \t [{test_data.min()}-{test_data.max()}]"
    )
    print("------------------------------------------------------------\n")
